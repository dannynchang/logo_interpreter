...
def precedence(infix):
    """Returns the precedence of an infix symbol.
    ...
    2
    """
    length = len(INFIX_GROUPS)
    for item in range(length):
        if infix in INFIX_GROUPS[item]:
            return item

def logo_eval(line, env, prev_token = None):
    """Evaluate the first expression in a line.
    ...
    """
    item = line.current
    arg = eval_noninfix(line, env)
    while type(item) == str and item in INFIX_SYMBOLS.keys():
        if precedence(item) <= precedence(prev_token):
            return arg
        elif prev_token is None or precedence(prev_token) < precedence(item):
            line.pop()
            arg2 = logo_eval(line, env, item)
        proc = env.procedures.get(INFIX_SYMBOLS[item], None)
        arg = logo_apply(proc, [arg, arg2])
        item = line.current
    return arg
...
def logo_apply(proc, args):
    ...
    if proc.isprimitive:
        ...
        n_args = proc.arg_count
        body = proc.body
        new_frame = {}
        if n < 0:
            return None
        for num in range(n_args):
            new_frame[proc.formal_params[num]] = args[num]
        env.push_frame(new_frame)
        counter = 0
        for lst in body:
            line = Buffer(body[counter])
    ...
    def set_variable_value(self, symbol, val):
        ...
        while last_frame > 0:
            if symbol in self._frames[last_frame]:
                ...
                last_frame -= 1
        self._frames[0][symbol]=val #set it in global 
        # raise NotImplementedError
